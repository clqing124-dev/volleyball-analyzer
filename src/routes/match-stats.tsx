...
export function MatchStatsPage() {
  ...
  useEffect(() => {
    ...
    db.rallies.where('matchId').equals(matchId).toArray().then((r) => {
      setRallies(r);
    });
  }, [matchId]);
  ...
  useEffect(() => {
    if (rallies.length > 0 && match) {
      let filteredRallies = rallies;
      if (selectedSet !== 'all') {
        filteredRallies = rallies.filter((r) => r.setId === selectedSet);
      }

      const filters: StatFilters = {};
      if (filterPlayer) filters.playerNumber = filterPlayer;

      const result = computeMatchStatistics(filteredRallies, match.type, filters);
      setStats(result);
    }
  ...
  const totalOurScore = match.sets?.reduce((s: number, set: any) => s + set.ourScore, 0) || 0;
  const totalOppScore = match.sets?.reduce((s: number, set: any) => s + set.opponentScore, 0) || 0;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-4 pb-3 flex items-center gap-3 border-b border-slate-800">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-400">
            {match.homeTeamName} vs {match.awayTeamName} · 总比分 {totalOurScore}-{totalOppScore}
          </p>
        </div>
      </div>
      ...
      <div className="px-4 py-2 flex gap-2 items-center flex-wrap">
        ...
        >
          ...
          {match.sets?.map((s: any) => (
            <option key={s.id} value={s.id}>第{s.setNumber}局</option>
          ))}
        </select>
      ...
      <div className="px-4 pb-2">
        <div className="grid grid-cols-4 gap-2">
          ...
          <StatCard label="得分率" value={formatPct(stats.rallyWinRate)} unit="%" />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {activeTab === 'serve' && <ServeSection stats={stats} isOwn={match.type === 'own'} />}
        {activeTab === 'reception' && <ReceptionSection stats={stats} />}
        {activeTab === 'set' && <SetSection stats={stats} />}
        {activeTab === 'attack' && <AttackSection stats={stats} isOwn={match.type === 'own'} />}
        {activeTab === 'block_defense' && <BlockDefenseSection stats={stats} />}
        {activeTab === 'transition' && stats.transition && <TransitionSection stats={stats} />}
      </div>
    </div>
  );
}
...
// ============================================================
// Section components (reused from set-stats but with match-level data)
// ============================================================

function ServeSection({ stats, isOwn }: { stats: MatchStatistics; isOwn: boolean }) {
  return (
...
function ReceptionSection({ stats }: { stats: MatchStatistics }) {
  return (
    <div className="space-y-3">
...
function SetSection({ stats }: { stats: MatchStatistics }) {
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <StatCard label="传4号位率" value={formatPct(stats.set.position4Rate)} unit="%" sub={`${stats.set.distributionByZone[4]}次`} />
        <StatCard label="传2号位率" value={formatPct(stats.set.position2Rate)} unit="%" sub={`${stats.set.distributionByZone[2]}次`} />
        <StatCard label="传3号位率" value={formatPct(stats.set.position3Rate)} unit="%" sub={`${stats.set.distributionByZone[3]}次`} />
        <StatCard label="传6号位率" value={formatPct(stats.set.position6Rate)} unit="%" sub={`${stats.set.distributionByZone[6]}次`} />
...
function AttackSection({ stats, isOwn }: { stats: MatchStatistics; isOwn: boolean }) {
  return (
    <div className="space-y-3">
...
function BlockDefenseSection({ stats }: { stats: MatchStatistics }) {
  return (
    <div className="space-y-3">
...
function TransitionSection({ stats }: { stats: MatchStatistics }) {
  if (!stats.transition) return null;
  return (
    <div className="space-y-3">
